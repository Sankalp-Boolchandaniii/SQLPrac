...
-- see all apartments and their residents
select r.first_name, a.apartment_id, a.floor_number, a.wing_name 
from residents r right join apartments a 
on r.apartment_id=a.apartment_id;

-- EXERCISE 1: Finding Unoccupied Apartments
select a.apartment_id from 
residents r right join apartments a 
on a.apartment_id=r.apartment_id 
where r.resident_id is NULL;

-- EXERCISE 2: Count the number of residents per apartment
select a.apartment_id, count(r.resident_id), group_concat(r.first_name) 
from residents r right join apartments a 
on a.apartment_id=r.apartment_id 
